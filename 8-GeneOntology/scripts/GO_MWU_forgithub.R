...
# Updated Hannah Aichelman October 2019
################################################################
...
#repeated this for both the 2 and 4 population files

#To do this with the 4-population files, load these:
# input <- read.csv("4Pop_GO_unique_dupsremoved.csv")
# annotations <- read.csv("Assembly_GoodCoralSymbiont_genenames.csv")
# newinput <- merge(input,annotations,by="gene",all=T)
# write.csv(newinput, "4Pop_GO_unique_dupsremoved_AllGenes.csv", row.names=F)

# And to do analysis with the 2-population host files, load these:
# Edit these to match your data file names: 
input="2Pop_GO_unique_dupsremoved_AllGenes.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
...
goDivision="CC" # either MF, or BP, or CC, change to look at different categories
