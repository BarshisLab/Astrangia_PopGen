...
# this loop is for extracting the column indexes for the column names specified on the first line of the stufffornr.txt file
			...
			for aline in cols2extract:
				...
				if extractlinecount==1:
					...
					alignlen=cols.index(words[2])
					percentmatch=cols.index(words[3])
		if linenum >1:
			cols[0]=cols[0].split(' ')[0]
			if cols[0] == contig:
#				print line
				d[cols[0]].append('%s\t%s\t%s\t%s' %(cols[hitdescription],cols[nrEval],cols[alignlen],cols[percentmatch]))
			else: 
...
####this step loops through the entries in your contig dictionary
...
for key,value in innames.items():
	if d.has_key(key):
		...
		innames[key]+='\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' %('No_sig_nr_hit','No_sig_nr_hit','No_sig_nr_hit','No_sig_nr_hit','No_sig_nr_hit','No_sig_nr_hit','No_sig_nr_hit','No_sig_nr_hit')
...
def extractGO(contigname):
	if uniprotIDs.has_key(contigname):
		...
		outstring='\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' %(ID, DE, KEGG, KEGGKO, PFAM[3:], GOcodes, GOBP, GOMF, GOCC, keywords)
		nomatch=0
	else:
		nomatch=1
		outstring='\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' %('No_Uniprotmatch','No_%_identity','No_evalue','No_ID','No_Description','No_KEGG', 'No_KEGGKO', 'No_PFAM', 'No_GO','No_GOCC','No_GOBP','No_GOMF','No_keywords')
...
o.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' %('ContigName', 'ContigLength', 'topnrMatch','topnrEvalue', 'topnrAlignmentLength', 'topnrPercMatch', 'nobadwordnrMatch', 'nobadwordnrEvalue', 'nobadwordnrAlignLength','nobadwordnrPercentMatch','Uniprotmatch','%_identity','evalue','ID','Description','KEGG', 'KEGGKO','PFAM','GO','GOCC','GOBP','GOMF','Keywords')) #used if you want a specific header and filename for each file
